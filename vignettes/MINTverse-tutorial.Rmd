...
> **Note:** This tutorial is an R adaptation of the original [MINTverse Python tutorial](https://github.com/CosmoNaught/MINTverse-tutorial/blob/main/MINTverse-tutorial.ipynb), which demonstrates how the underlying Python `minte` package works. The examples and workflow here mirror that tutorial but use the **rminte** R interface.
...
MINTe provides a convenience function `create_scenario_plots_mpl` that wraps the
Python matplotlib plotting functionality:
...
**Note:** For native R plotting with ggplot2, you can also use `plot_prevalence()` 
or `plot_cases()` which provide the same functionality but return ggplot2 objects
that integrate better with R workflows.
